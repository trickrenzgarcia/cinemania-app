...
export default function GCashSuccess({ searchParams: { message, total }} : { searchParams: { message: string, total: string }}) {
        ...
        <div className='flex w-96 mx-auto'>
          <div className='mt-16  w-full bg-white shadow-md'>
            ...
            <div className='w-full flex flex-col items-center p-7 justify-center'>
              <h2 className='font-semibold mb-10 text-green-500'>Successful Purchase</h2>
              <h2 className='mb-16'>{message}</h2>
              <Button className='bg-sky-500 px-11 rounded-full' onClick={handleOnClick}>Go back</Button>
            </div>
          </div>
        </div>
      </div>
